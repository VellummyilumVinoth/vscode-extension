...
import { ComponentInfo, DataMapperMetadata, Diagnostics, ImportStatements } from "../..";
...
export interface FunctionDefinitionFromSyntaxTree {
    filePath: string;
    functionName: string;
}

export interface MappingGenerationRequest {
    parameters: MappingParameters;
    metadata?: ExtendedDataMapperMetadata;
    attachments?: Attachment[];
}
...
export interface ProcessMappingResult {
    matchingFunction: ComponentInfo[];
    fileArray: Array<{ filePath: string; content: string }>;
    inputParams: string[];
    outputParam: string;
    existingFunctionMatch: boolean;
    filePath: string;
...
export interface RepairedFilesContent {
    finalContent: string;
    customFunctionsContent?: string;
}
