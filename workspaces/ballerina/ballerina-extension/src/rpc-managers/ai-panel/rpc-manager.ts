...
User reverted the last made changes. The files have been restored to the state before this generation.
    ...
    async enableSkillFromChat(params: SkillEnableRequest): Promise<boolean> {
        try {
            const configPath = this.resolveSkillsConfigPath(SkillTier.PROJECT, resolveProjectRootPath());
            setSkillEnabled(configPath, params.skillId, true);
    ...
    async cancelSkillEnable(params: SkillEnableCancelRequest): Promise<void> {
        sendSkillEnableNotification({ type: "skill_enable_event", requestId: params.requestId, stage: SkillEnableStage.SKIPPED, skillName: '', skillId: '' } as any);
        approvalManager.resolveSkillEnable(params.requestId, false);
    }
    ...
    async parseSkillFile(params: ParseSkillFileRequest): Promise<ParseSkillFileResponse> {
        try {
            ...
            if (ext === '.md') {
                ...
                if (!name || !trigger) {
                    return { name: '', trigger: '', error: 'Missing name or description in YAML front matter.' };
                }
            ...
            if (ext === '.zip' || ext === '.skill') {
                ...
                if (!entry) {
                    return { name: '', trigger: '', error: 'No SKILL.md found inside the archive.' };
                }
                const raw = (await entry.buffer()).toString('utf-8');
                const { name, trigger, content } = parseSkillMd(raw);
                if (!name || !trigger) {
                    return { name: '', trigger: '', error: 'Missing name or description in SKILL.md.' };
                }
                return { name, trigger, body: content !== trigger ? content : undefined };
            }

            return { name: '', trigger: '', error: 'Unsupported file type. Use .md, .zip, or .skill.' };
        } catch (err: any) {
            return { name: '', trigger: '', error: err?.message ?? 'Failed to parse skill file.' };
        }
    }
